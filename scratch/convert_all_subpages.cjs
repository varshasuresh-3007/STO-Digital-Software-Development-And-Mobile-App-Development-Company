...
function convertToJSX(html) {
  ...
  // 3. Self-close void elements correctly (robust regex)
  const voidElements = ['img', 'br', 'input', 'source', 'hr', 'meta', 'link'];
  voidElements.forEach(tag => {
    const regex = new RegExp('<' + tag + '\\b([^>]*?)(?:\\s*\\/)?\\s*>', 'gi');
...
function processDirectory(dir, relativePath = '') {
  ...
  for (const item of items) {
    ...
    if (stats.isDirectory()) {
      ...
      // Strip all HTML comment nodes recursively
      $('*').contents().filter(function() {
        return this.nodeType === 8; // 8 represents Comment
      }).remove();
      ...
      // Extract unique contents, while actively removing nested script and style elements
      let mainHtml = '';
      if ($('main').length > 0) {
        const mainClone = $('main').clone();
        mainClone.find('script, style').remove();
        mainHtml = mainClone.html();
      } else {
        // Fallback to body tag extraction while stripping out headers/footers/scripts
